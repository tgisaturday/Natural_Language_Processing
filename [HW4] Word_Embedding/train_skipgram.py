...
class SkipGram(nn.Module):
  ...
  def __init__(self, vocab_size, embedding_dim, window_size):
    ...
    for i in range(window_size-1):
        linears.append(nn.Linear(embedding_dim, vocab_size))
...
if not os.path.exists('embeddings/SkipGram/'):
    os.makedirs('embeddings/SkipGram/')
# training loop
for epoch in range(10):
  ...
  torch.save(model.state_dict(), "models/SkipGram/epoch{}.model".format(epoch))
...
f_embed = open("embeddings/SkipGram/embedding.pkl","wb")
pickle.dump(embedding_matrix,f_embed)
f_embed.close()

f_dict= open("embeddings/SkipGram/word_to_ix.pkl","wb")
